...
def get_track_plot(data_path, track_mst_path):
    # データフレームとして読み込み
    df = pd.read_excel(data_path)
    # dfのキュービック縦横高さ重複識別モデルを変更する(受入が違うものを混載させてはいけないのでラベルを分けています)
    df["キュービック縦横高さ重複識別ラベル"] = \
        df["キュービック_たて"].astype(str) +"_"+ df["キュービック_よこ"].astype(str) +"_"+ df["キュービック_高さ"].astype(str) + "_" + df["整備室"].astype(str)
    ...
    # 整備室は受入と同じ意味
    df_amari["余り箱_識別ラベル"] = \
        df_amari["キュービック_たて"].astype(str) +"_"+ df_amari["キュービック_よこ"].astype(str) +"_"+ round(df_amari["混載条件のパレット含めた高さ"], 3).astype(str) + "_" + df_amari["整備室"].astype(str)
    ...
    # 綺麗なキュービックと混載して綺麗になったキュービックの数を足し合わせて　df_duplicate　に更新する
    ...
    df_amaribako["キュービック_高さ"] = df_amaribako["混載条件のパレット含めた高さ"]
    ...
    fig_2d = plotter.plot_2d(visualize_type="確定")
